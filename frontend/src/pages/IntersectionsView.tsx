...
import { intersectionsApi, questionsApi, tagsApi } from '@/api/client';
...
export default function IntersectionsView({ intersectionId }: IntersectionsViewProps) {
  const queryClient = useQueryClient();
  const [editingQuestion, setEditingQuestion] = useState<number | null>(null);
  const [editingTags, setEditingTags] = useState<string[]>([]);

  const { data: questions, isLoading } = useQuery({
    queryKey: ['intersectionQuestions', intersectionId],
    queryFn: () => intersectionsApi.getIntersectionQuestions(intersectionId),
  ...
  const updateQuestionMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['intersectionQuestions', intersectionId] });
    },
  });
  ...
  if (!intersectionId) {
    return (
      <Card>
        <p className="text-text-muted text-center">Select an intersection to view questions</p>
      </Card>
    );
  }
  ...
  const completedCount = questions?.filter((q) => q.done).length || 0;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-3xl font-bold text-blue">{totalCount}</p>
            <p className="text-sm text-text-muted">Total Questions</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-green">{completedCount}</p>
            <p className="text-sm text-text-muted">Completed</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-mauve">{percentage.toFixed(1)}%</p>
